...
export default function Certifications(){
 ...
 <div className="card table-wrap"><table className="data-table"><thead><tr><th>Certification</th><th>Issuer</th><th>Issue</th><th>Expiry</th><th>Actions</th></tr></thead><tbody>{items.length?items.map(x=><tr key={x._id}><td><strong>{x.certificationName}</strong><div className="chips">{(x.skills||[]).map(s=><span className="chip" key={s}>{s}</span>)}</div></td><td>{x.issuingOrganization}</td><td>{x.issueDate?.slice(0,10)}</td><td>{x.doesNotExpire?"No expiry":x.expiryDate?.slice(0,10)||"—"}</td><td><span className="badge">{x.verificationStatus}</span></td><td><button className="btn btn-secondary" onClick={()=>edit(x)}>Edit</button>{" "}<button className="btn btn-danger" onClick={()=>del(x._id)}>Delete</button></td></tr>):<tr><td colSpan="6">No certifications added yet.</td></tr>}</tbody></table></div>
 </div>;
}
